...
(cl:defun documentationp (expr)
  (stringp expr))

(cl:defun declarationp (expr)
  (and (listp expr)
       (eq (car expr) 'declare)))

(cl:defun check-component-symbols (symbols components)
...
(cl:defmacro def-with-components (name &rest components)
...
(cl:defun defclass-class-name (defclass-source)
  (cadr defclass-source))

(cl:defun defclass-superclass-names (defclass-source)
  (caddr defclass-source))

(cl:defun defclass-slot-specifiers (defclass-source)
  (cadddr defclass-source))

(cl:defun defclass-slot-names (defclass-source)
...
(cl:defun defclass-slot-options (defclass-source)
  (let ((slot-specifiers (defclass-slot-specifiers defclass-source)))
    (and slot-specifiers
	 (loop for slot-specifier in slot-specifiers
	       collect (if (listp slot-specifier)
...
(cl:defun defclass-reader-function-names (defclass-source)
...
(cl:defun defclass-writer-function-names (defclass-source)
  (let ((slot-options (defclass-slot-options defclass-source)))
    (and slot-options
	 (loop for slot-option in slot-options
	       collect (and slot-option
...
(cl:defun defclass-accessor-function-names (defclass-source)
  (let ((slot-options (defclass-slot-options defclass-source)))
    (and slot-options
	 (loop for slot-option in slot-options
	       collect (and slot-option
...
(cl:defun defclass-allocation-types (defclass-source)
  (let ((slot-options (defclass-slot-options defclass-source)))
    (and slot-options
	 (loop for slot-option in slot-options
	       collect (and slot-option
...
(cl:defun defclass-initarg-names (defclass-source)
  (let ((slot-options (defclass-slot-options defclass-source)))
    (and slot-options
	 (loop for slot-option in slot-options
	       collect (and slot-option
...
(cl:defun defclass-initforms (defclass-source)
  (let ((slot-options (defclass-slot-options defclass-source)))
    (and slot-options
	 (loop for slot-option in slot-options
	       collect (and slot-option
...
(cl:defun defclass-type-specifiers (defclass-source)
  (let ((slot-options (defclass-slot-options defclass-source)))
    (and slot-options
	 (loop for slot-option in slot-options
	       collect (and slot-option
...
(cl:defun defclass-slot-documentations (defclass-source)
  (let ((slot-options (defclass-slot-options defclass-source)))
    (and slot-options
	 (loop for slot-option in slot-options
	       collect (and slot-option
...
(cl:defun defclass-class-options (defclass-source)
  (cdddr defclass-source))

(cl:defun defclass-default-initargs (defclass-source)
  (let ((class-options (defclass-class-options defclass-source)))
    (and class-options
	 (cdr (member :default-initargs class-options :key #'car)))))

(cl:defun defclass-documentation (defclass-source)
  (let ((class-options (defclass-class-options defclass-source)))
    (and class-options
	 (cadr (member :documentation class-options :key #'car)))))

(cl:defun defclass-metaclass (defclass-source)
  (let ((class-options (defclass-class-options defclass-source)))
    (and class-options
...
(cl:defun defconstant-name (defconstant-source)
  (cadr defconstant-source))

(cl:defun defconstant-initial-value (defconstant-source)
  (caddr defconstant-source))

(cl:defun defconstant-documentation (defconstant-source)
...
(cl:defun defgeneric-function-name (defgeneric-source)
  (cadr defgeneric-source))

(cl:defun defgeneric-gf-lambda-list (defgeneric-source)
  (caddr defgeneric-source))

(cl:defun defgeneric-options (defgeneric-source)
  (remove-if (lambda (x)
	       (eq (car x) :method))
	     (cdddr defgeneric-source)))

(cl:defun defgeneric-argument-precedence-order (defgeneric-source)
  (let ((options (defgeneric-options defgeneric-source)))
    (and options
	 (cdr (member :argument-precedence-order options :key #'car)))))

(cl:defun defgeneric-gf-declarations (defgeneric-source)
  (let ((options (defgeneric-options defgeneric-source)))
    (and options
	 (cdr (member 'cl:declare options :key #'car)))))

(cl:defun defgeneric-documentation (defgeneric-source)
  (let ((options (defgeneric-options defgeneric-source)))
    (and options
	 (cadr (member :documentation options :key #'car)))))

(cl:defun defgeneric-method-combination (defgeneric-source)
  (let ((options (defgeneric-options defgeneric-source)))
    (and options
	 (cdr (member :method-combination options :key #'car)))))

(cl:defun defgeneric-generic-function-class (defgeneric-source)
  (let ((options (defgeneric-options defgeneric-source)))
    (and options
	 (cadr (member :generic-function-class options :key #'car)))))

(cl:defun defgeneric-method-class (defgeneric-source)
  (let ((options (defgeneric-options defgeneric-source)))
    (and options
	 (cadr (member :method-class options :key #'car)))))

(cl:defun defgeneric-method-descriptions (defgeneric-source)
...
(cl:defun define-compiler-macro-name (define-compiler-macro-source)
  (cadr define-compiler-macro-source))

(cl:defun define-compiler-macro-lambda-list (define-compiler-macro-source)
  (caddr define-compiler-macro-source))

(cl:defun define-compiler-macro-declarations (define-compiler-macro-source)
...
(cl:defun define-compiler-macro-documentation (define-compiler-macro-source)
  (let ((post-lambda-list (cdddr define-compiler-macro-source)))
    (loop for expr in post-lambda-list
	  while (or (declarationp expr)
		    (documentationp expr))
...
(cl:defun define-compiler-macro-forms (define-compiler-macro-source)
  (let ((post-lambda-list (cdddr define-compiler-macro-source)))
...
(cl:defun define-condition-name (define-condition-source)
  (cadr define-condition-source))

(cl:defun define-condition-parent-types (define-condition-source)
  (caddr define-condition-source))

(cl:defun define-condition-slot-specs (define-condition-source)
  (cadddr define-condition-source))

(cl:defun define-condition-slot-names (define-condition-source)
...
(cl:defun define-condition-slot-options (define-condition-source)
  (let ((slot-specs (define-condition-slot-specs define-condition-source)))
    (and slot-specs
	 (loop for slot-spec in slot-specs
	       if (symbolp slot-spec)
...
(cl:defun define-condition-slot-readers (define-condition-source)
...
(cl:defun define-condition-slot-writers (define-condition-source)
  (let ((slot-options (define-condition-slot-options define-condition-source)))
    (and slot-options
	 (loop for slot-option in slot-options
	       collect (and slot-option
...
(cl:defun define-condition-slot-accessors (define-condition-source)
  (let ((slot-options (define-condition-slot-options define-condition-source)))
    (and slot-options
	 (loop for slot-option in slot-options
	       collect (and slot-option
...
(cl:defun define-condition-slot-allocations (define-condition-source)
  (let ((slot-options (define-condition-slot-options define-condition-source)))
    (and slot-options
	 (loop for slot-option in slot-options
	       collect (and slot-option
...
(cl:defun define-condition-slot-initargs (define-condition-source)
  (let ((slot-options (define-condition-slot-options define-condition-source)))
    (and slot-options
	 (loop for slot-option in slot-options
	       collect (and slot-option
...
(cl:defun define-condition-slot-initforms (define-condition-source)
  (let ((slot-options (define-condition-slot-options define-condition-source)))
    (and slot-options
	 (loop for slot-option in slot-options
	       collect (and slot-option
...
(cl:defun define-condition-slot-types (define-condition-source)
  (let ((slot-options (define-condition-slot-options define-condition-source)))
    (and slot-options
	 (loop for slot-option in slot-options
	       collect (and slot-option
...
(cl:defun define-condition-options (define-condition-source)
  (cddddr define-condition-source))

(cl:defun define-condition-default-initargs (define-condition-source)
  (let ((options (define-condition-options define-condition-source)))
    (and options
	 (cdr (member :default-initargs options :key #'car)))))

(cl:defun define-condition-documentation (define-condition-source)
  (let ((options (define-condition-options define-condition-source)))
    (and options
	 (cadr (member :documentation options :key #'car)))))

(cl:defun define-condition-report-name (define-condition-source)
  (let ((options (define-condition-options define-condition-source)))
    (and options
...
(cl:defun define-method-combination-name (define-method-combination-source)
  (cadr define-method-combination-source))

(cl:defun define-method-combination-short-form-options (define-method-combination-source)
...
(cl:defun define-method-combination-identity-with-one-argument (define-method-combination-source)
...
(cl:defun define-method-combination-operator (define-method-combination-source)
  (let ((short-form-options (define-method-combination-short-form-options define-method-combination-source)))
    (and short-form-options
	 (cadr (member :operator short-form-options)))))

(cl:defun define-method-combination-lambda-list (define-method-combination-source)
  (caddr define-method-combination-source))

(cl:defun define-method-combination-method-group-specifiers (define-method-combination-source)
  (cadddr define-method-combination-source))

(cl:defun define-method-combination-method-group-specifier-names (define-method-combination-source)
...
(cl:defun define-method-combination-qualifier-patterns (define-method-combination-source)
  (let ((method-group-specifiers (define-method-combination-method-group-specifiers define-method-combination-source)))
    (and method-group-specifiers
...
(cl:defun define-method-combination-predicates (define-method-combination-source)
  (let ((method-group-specifiers (define-method-combination-method-group-specifiers define-method-combination-source)))
    (and method-group-specifiers
	 (loop for method-group-specifier in method-group-specifiers
...
(cl:defun define-method-combination-long-form-options (define-method-combination-source)
  (let ((method-group-specifiers (define-method-combination-method-group-specifiers define-method-combination-source)))
    (and method-group-specifiers
	 (loop for method-group-specifier in method-group-specifiers
...
(cl:defun define-method-combination-descriptions (define-method-combination-source)
...
(cl:defun define-method-combination-orders (define-method-combination-source)
  (let ((long-form-options (define-method-combination-long-form-options define-method-combination-source)))
    (and long-form-options
	 (loop for long-form-option in long-form-options
	       collect (cadr (member :order long-form-option))))))

(cl:defun define-method-combination-requireds (define-method-combination-source)
  (let ((long-form-options (define-method-combination-long-form-options define-method-combination-source)))
    (and long-form-options
	 (loop for long-form-option in long-form-options
	       collect (cadr (member :required long-form-option))))))

(cl:defun define-method-combination-arguments (define-method-combination-source)
...
(cl:defun define-method-combination-generic-function (define-method-combination-source)
...
(cl:defun define-method-combination-declarations (define-method-combination-source)
...
(cl:defun define-method-combination-documentation (define-method-combination-source)
  (let ((short-form-options (define-method-combination-short-form-options define-method-combination-source)))
...
(cl:defun define-method-combination-forms (define-method-combination-source)
...
(cl:defun define-modify-macro-name (define-modify-macro-source)
  (cadr define-modify-macro-source))

(cl:defun define-modify-macro-lambda-list (define-modify-macro-source)
  (caddr define-modify-macro-source))

(cl:defun define-modify-macro-function (define-modify-macro-source)
  (cadddr define-modify-macro-source))

(cl:defun define-modify-macro-documentation (define-modify-macro-source)
...
(cl:defun define-setf-expander-access-fn (define-setf-expander-source)
  (cadr define-setf-expander-source))

(cl:defun define-setf-expander-lambda-list (define-setf-expander-source)
  (caddr define-setf-expander-source))

(cl:defun define-setf-expander-declarations (define-setf-expander-source)
...
(cl:defun define-setf-expander-documentation (define-setf-expander-source)
  (let ((post-lambda-list (cdddr define-setf-expander-source)))
    (loop for expr in post-lambda-list
	  while (or (declarationp expr)
		    (documentationp expr))
...
(cl:defun define-setf-expander-forms (define-setf-expander-source)
  (let ((post-lambda-list (cdddr define-setf-expander-source)))
    (loop for expr on post-lambda-list
	  while (or (declarationp (car expr))
		    (documentationp (car expr)))
...
(cl:defun define-symbol-macro-symbol (define-symbol-macro-source)
  (cadr define-symbol-macro-source))

(cl:defun define-symbol-macro-expansion (define-symbol-macro-source)
...
(cl:defun defmacro-name (defmacro-source)
  (cadr defmacro-source))

(cl:defun defmacro-lambda-list (defmacro-source)
  (caddr defmacro-source))

(cl:defun defmacro-declarations (defmacro-source)
...
(cl:defun defmacro-documentation (defmacro-source)
  (let ((post-lambda-list (cdddr defmacro-source)))
    (loop for expr in post-lambda-list
	  while (or (declarationp expr)
		    (documentationp expr))
...
(cl:defun defmacro-forms (defmacro-source)
  (let ((post-lambda-list (cdddr defmacro-source)))
    (loop for expr on post-lambda-list
	  while (or (declarationp (car expr))
		    (documentationp (car expr)))
...
(cl:defun defmethod-function-name (defmethod-source)
  (cadr defmethod-source))

(cl:defun defmethod-method-qualifiers (defmethod-source)
  (loop for qualifier in (cddr defmethod-source)
	while (not (listp qualifier))
	collect qualifier))

(cl:defun defmethod-specialized-lambda-list (defmethod-source)
  (loop for possible-lambda-list in (cddr defmethod-source)
	when (listp possible-lambda-list)
	  return possible-lambda-list))

(cl:defun defmethod-declarations (defmethod-source)
...
(cl:defun defmethod-documentation (defmethod-source)
  (loop for rest-defmethod-source on (cddr defmethod-source)
	when (listp (car rest-defmethod-source))
...
(cl:defun defmethod-forms (defmethod-source)
  (loop for rest-defmethod-source on (cddr defmethod-source)
	when (listp (car rest-defmethod-source))
...
(cl:defun defpackage-options (defpackage-source)
  (cddr defpackage-source))

(cl:defun defpackage-defined-package-name (defpackage-source)
  (cadr defpackage-source))

(cl:defun defpackage-nicknames (defpackage-source)
  (let ((options (defpackage-options defpackage-source)))
    (mapcan #'cdr (remove-if-not (lambda (x)
				   (eq (car x) :nicknames))
				 options))))

(cl:defun defpackage-documentation (defpackage-source)
  (let ((options (defpackage-options defpackage-source)))
    (and options
	 (cadr (member :documentation options)))))

(cl:defun defpackage-use-package-names (defpackage-source)
  (let ((options (defpackage-options defpackage-source)))
    (mapcan #'cdr (remove-if-not (lambda (x)
				   (eq (car x) :use))
				 options))))

(cl:defun defpackage-shadow-symbol-names (defpackage-source)
  (let ((options (defpackage-options defpackage-source)))
    (mapcan #'cdr (remove-if-not (lambda (x)
				   (eq (car x) :shadow))
				 options))))

(cl:defun defpackage-shadowing-import-from-package-names (defpackage-source)
  (let ((options (defpackage-options defpackage-source)))
    (mapcar #'cadr (remove-if-not (lambda (x)
				   (eq (car x) :shadowing-import-from))
				  options))))

(cl:defun defpackage-shadowing-import-from-symbol-names (defpackage-source)
  (let ((options (defpackage-options defpackage-source)))
    (mapcan #'cddr (remove-if-not (lambda (x)
				   (eq (car x) :shadowing-import-from))
				 options))))

(cl:defun defpackage-import-from-package-names (defpackage-source)
  (let ((options (defpackage-options defpackage-source)))
    (mapcar #'cadr (remove-if-not (lambda (x)
				   (eq (car x) :import-from))
				  options))))

(cl:defun defpackage-import-from-symbol-names (defpackage-source)
  (let ((options (defpackage-options defpackage-source)))
    (mapcan #'cddr (remove-if-not (lambda (x)
				   (eq (car x) :import-from))
				  options))))

(cl:defun defpackage-export-symbol-names (defpackage-source)
  (let ((options (defpackage-options defpackage-source)))
    (mapcan #'cdr (remove-if-not (lambda (x)
				   (eq (car x) :export))
				 options))))

(cl:defun defpackage-import-symbol-names (defpackage-source)
  (let ((options (defpackage-options defpackage-source)))
    (mapcan #'cdr (remove-if-not (lambda (x)
				   (eq (car x) :export))
				 options))))

(cl:defun defpackage-size (defpackage-source)
  (let ((options (defpackage-options defpackage-source)))
...
(cl:defun defparameter-name (defparameter-source)
  (cadr defparameter-source))

(cl:defun defparameter-initial-value (defparameter-source)
  (caddr defparameter-source))

(cl:defun defparameter-documentation (defparameter-source)
...
(cl:defun defsetf-access-fn (defsetf-source)
  (cadr defsetf-source))

(cl:defun defsetf-update-fn (defsetf-source)
  (let ((possible-update-fn (caddr defsetf-source)))
    (if (symbolp possible-update-fn)
	possible-update-fn)))

(cl:defun defsetf-documentation (defsetf-source)
...
(cl:defun defstruct-name-and-options (defstruct-source)
  (cadr defstruct-source))

(cl:defun defstruct-structure-name (defstruct-source)
  (let ((name-and-options (defstruct-name-and-options defstruct-source)))
    (if (listp name-and-options)
	(car name-and-options)
	name-and-options)))

(cl:defun defstruct-options (defstruct-source)
  (let ((name-and-options (defstruct-name-and-options defstruct-source)))
    (if (listp name-and-options)
	(cdr name-and-options)
	nil)))

(cl:defun defstruct-conc-name-option (defstruct-source)
...
(cl:defun defstruct-constructor-options (defstruct-source)
  (let ((options (defstruct-options defstruct-source)))
    (remove-if-not (lambda (x)
		     (if (listp x)
...
(cl:defun defstruct-copier-option (defstruct-source)
  (let ((options (defstruct-options defstruct-source)))
    (if (not (null options))
...
(cl:defun defstruct-predicate-option (defstruct-source)
  (let ((options (defstruct-options defstruct-source)))
    (if (not (null options))
...
(cl:defun defstruct-include-option (defstruct-source)
  (let ((options (defstruct-options defstruct-source)))
    (if (not (null options))
...
(cl:defun defstruct-printer-option (defstruct-source)
  (let ((options (defstruct-options defstruct-source)))
    (if (not (null options))
...
(cl:defun defstruct-print-object-option (defstruct-source)
...
(cl:defun defstruct-print-function-option (defstruct-source)
  (let ((printer-option (defstruct-printer-option defstruct-source)))
    (if (and (not (null printer-option))
...
(cl:defun defstruct-type-option (defstruct-source)
  (let ((options (defstruct-options defstruct-source)))
    (if (not (null options))
	(car (member :type options :key #'car))
	nil)))

(cl:defun defstruct-named-option (defstruct-source)
  (let ((options (defstruct-options defstruct-source)))
    (if (not (null options))
	(car (member :named options))
	nil)))

(cl:defun defstruct-initial-offset-option (defstruct-source)
  (let ((options (defstruct-options defstruct-source)))
    (if (not (null options))
	(car (member :initial-offset options :key #'car))
	nil)))

(cl:defun defstruct-slot-descriptions (defstruct-source)
  (let ((possible-descriptions (cddr defstruct-source)))
    (if (documentationp (car possible-descriptions))
	(cdr possible-descriptions)
	possible-descriptions)))

(cl:defun defstruct-slot-options (defstruct-source)
...
(cl:defun defstruct-conc-name (defstruct-source)
...
(cl:defun defstruct-constructor-arglists (defstruct-source)
...
(cl:defun defstruct-constructor-names (defstruct-source)
  (let ((constructor-options (defstruct-constructor-options defstruct-source)))
    (and constructor-options
	 (loop for constructor-option in constructor-options
	       collect (if (listp constructor-option)
...
(cl:defun defstruct-copier-name (defstruct-source)
...
(cl:defun defstruct-included-structure-name (defstruct-source)
  (let ((include-option (defstruct-include-option defstruct-source)))
    (and include-option
	 (cadr include-option))))

(cl:defun defstruct-include-option-slot-descriptors (defstruct-source)
  (let ((include-option (defstruct-include-option defstruct-source)))
    (and include-option
	 (cddr include-option))))

(cl:defun defstruct-initial-offset (defstruct-source)
...
(cl:defun defstruct-predicate-name (defstruct-source)
  (let ((predicate-option (defstruct-predicate-option defstruct-source)))
    (and predicate-option
	 (cadr predicate-option))))

(cl:defun defstruct-printer-name (defstruct-source)
  (let ((printer-option (defstruct-printer-option defstruct-source)))
    (and printer-option
	 (cadr printer-option))))

(cl:defun defstruct-slot-names (defstruct-source)
  (let ((slot-descriptions (defstruct-slot-descriptions defstruct-source)))
...
(cl:defun defstruct-slot-initforms (defstruct-source)
  (let ((slot-descriptions (defstruct-slot-descriptions defstruct-source)))
    (and slot-descriptions
	 (loop for slot-description in slot-descriptions
	       collect (if (listp slot-description)
...
(cl:defun defstruct-slot-types (defstruct-source)
  (let ((slot-options (defstruct-slot-options defstruct-source)))
    (and slot-options
	 (loop for slot-option in slot-options
	       collect (cadr (member :type slot-option))))))

(cl:defun defstruct-slot-read-only-ps (defstruct-source)
  (let ((slot-options (defstruct-slot-options defstruct-source)))
    (and slot-options
	 (loop for slot-option in slot-options
	       collect (cadr (member :read-only slot-option))))))

(cl:defun defstruct-type (defstruct-source)
  (let ((type-option (defstruct-type-option defstruct-source)))
    (and type-option
	 (cadr type-option))))

(cl:defun defstruct-documentation (defstruct-source)
...
(cl:defun deftype-name (deftype-source)
  (cadr deftype-source))

(cl:defun deftype-lambda-list (deftype-source)
  (caddr deftype-source))

(cl:defun deftype-declarations (deftype-source)
...
(cl:defun deftype-documentation (deftype-source)
  (let ((post-lambda-list (cdddr deftype-source)))
    (loop for expr in post-lambda-list
	  while (or (declarationp expr)
		    (documentationp expr))
...
(cl:defun deftype-forms (deftype-source)
  (let ((post-lambda-list (cdddr deftype-source)))
    (loop for expr on post-lambda-list
	  while (or (declarationp (car expr))
		    (documentationp (car expr)))
...
(adp:subsubheader "Cl:Defun components")

(cl:defun cl:defun-function-name (cl:defun-source)
  (cadr cl:defun-source))

(cl:defun cl:defun-lambda-list (cl:defun-source)
  (caddr cl:defun-source))

(cl:defun cl:defun-declarations (cl:defun-source)
  (let ((post-lambda-list (cdddr cl:defun-source)))
    (loop for expr in post-lambda-list
	  while (or (declarationp expr)
		    (documentationp expr))
	  when (declarationp expr)
...
(cl:defun cl:defun-documentation (cl:defun-source)
  (let ((post-lambda-list (cdddr cl:defun-source)))
    (loop for expr in post-lambda-list
	  while (or (declarationp expr)
		    (documentationp expr))
...
(cl:defun cl:defun-forms (cl:defun-source)
  (let ((post-lambda-list (cdddr cl:defun-source)))
    (loop for expr on post-lambda-list
	  while (or (declarationp (car expr))
		    (documentationp (car expr)))
...
(def-with-components cl:defun function-name lambda-list declarations documentation forms)
...
(cl:defun defvar-name (defvar-source)
  (cadr defvar-source))

(cl:defun defvar-initial-value (defvar-source)
  (caddr defvar-source))

(cl:defun defvar-documentation (defvar-source)
